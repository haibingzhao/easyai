...
import { ArrowLeft, Play, History, StopCircle, PauseCircle, X, Globe, ChevronDown, Check } from 'lucide-react';
...
export const DAGCanvas: React.FC<DAGCanvasProps> = ({ preset, onBack }) => {
  ...
  // Group model configs by groupId for the dropdown display
  const groupedModelConfigs = useMemo(() => {
    ...
    for (const mc of modelConfigs) {
      if (mc.groupId && groupNames[mc.groupId]) {
        const list = byGroup.get(mc.groupId) || [];
        list.push(mc);
        byGroup.set(mc.groupId, list);
      } else {
        ungrouped.push(mc);
      }
    }
    return { byGroup, ungrouped };
  }, [modelConfigs, groupNames]);
              ...
              <div className="mb-3">
                ...
                <div className="relative" ref={modelDropdownRef}>
                  <button
                    ...
                    className="flex items-center w-full h-8 px-2 text-sm rounded-md border border-input bg-transparent hover:bg-muted transition-colors"
                  >
                    <span className="flex-1 text-left truncate">
                      {selectedModelConfig ? selectedModelConfig.name : i18n('Default')}
                    </span>
                    <ChevronDown className={`w-3.5 h-3.5 text-muted-foreground transition-transform ${modelDropdownOpen ? 'rotate-180' : ''}`} />
                  </button>
                  {modelDropdownOpen && (
                    <div className="absolute top-full left-0 right-0 mt-1 max-h-[240px] overflow-y-auto bg-popover border border-border rounded-md shadow-lg z-50">
                      {/* Default option */}
                      <button
                        type="button"
                        onClick={() => {
                          setSelectedModelConfigId('');
                          storageService.saveSwarmPresetModel(preset.name, '');
                          setModelDropdownOpen(false);
                        }}
                        className={`w-full text-left px-3 py-2 text-sm hover:bg-muted transition-colors ${
                          !selectedModelConfigId ? 'bg-muted' : ''
                        }`}
                      >
                        <div className="flex items-center justify-between">
                          <span className="font-medium">{i18n('Default')}</span>
                          {!selectedModelConfigId && <Check className="w-3.5 h-3.5 text-primary" />}
                        </div>
                      </button>
                      {/* Grouped models */}
                      {[...groupedModelConfigs.byGroup.entries()].map(([groupId, configs]) => (
                        <div key={groupId}>
                          ...
                          {configs.map(mc => (
                            <button
                              ...
                              type="button"
                              onClick={() => {
                                setSelectedModelConfigId(mc.id);
                                storageService.saveSwarmPresetModel(preset.name, mc.id);
                                setModelDropdownOpen(false);
                              }}
                              className={`w-full text-left px-3 py-2 hover:bg-muted transition-colors ${
                                selectedModelConfigId === mc.id ? 'bg-muted' : ''
                              }`}
                            >
                              <div className="flex items-center justify-between">
                                <div className="min-w-0">
                                  <div className="text-sm font-medium truncate">{mc.name}</div>
                                  <div className="text-xs text-muted-foreground mt-0.5 truncate">{mc.modelName || mc.modelId}</div>
                                </div>
                                {selectedModelConfigId === mc.id && <Check className="w-3.5 h-3.5 text-primary shrink-0 ml-2" />}
                              </div>
                            </button>
                          ))}
                        </div>
                      ...
                      {/* Ungrouped models */}
                      {groupedModelConfigs.ungrouped.map(mc => (
                        <button
                          key={mc.id}
                          type="button"
                          onClick={() => {
